...
const parseCommits = (logOutput: string): ConventionalCommit[] => {
    return logOutput
        .split('\u001e')
...
const determineBump = (commits: ConventionalCommit[]): BumpType => {
    let hasMinor = false;
    let hasPatch = false;
...
const incrementVersion = (version: string, bump: Exclude<BumpType, 'none'>): string => {
