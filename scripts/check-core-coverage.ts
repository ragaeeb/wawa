...
const THRESHOLD = Number.parseFloat(process.env.CORE_COVERAGE_THRESHOLD ?? "80");
...
function parseLcov(lcov: string): CoverageTotals {
  ...
  for (const record of records) {
    ...
    if (!sourcePath.includes("src/core/")) continue;

    const lineFound = Number.parseInt(
      lines.find((line) => line.startsWith("LF:"))?.slice(3) ?? "0",
      10,
    );
    const lineHit = Number.parseInt(
      lines.find((line) => line.startsWith("LH:"))?.slice(3) ?? "0",
      10,
    );
    const fnFound = Number.parseInt(
      lines.find((line) => line.startsWith("FNF:"))?.slice(4) ?? "0",
      10,
    );
    const fnHit = Number.parseInt(
      lines.find((line) => line.startsWith("FNH:"))?.slice(4) ?? "0",
      10,
    );
