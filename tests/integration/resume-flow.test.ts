...
describe('resume flow integration', () => {
    it('should parse prior export, merge with new tweets, and create accurate top metadata', () => {
        ...
        expect(meta.merge_info?.duplicates_removed).toBe(1);
        expect(meta.scroll_responses_captured).toBe(32);
    });

    it('should use current-reported count when previous metadata has no valid reported count', () => {
        const priorExport = {
            meta: {
                username: 'example',
                export_started_at: '2026-02-01T10:00:00.000Z',
            ...
            items: [{ id: '1', created_at: '2026-01-01 00:00:00', text: 'old' }],
        };

        const parsed = parseResumeInput(priorExport);
        const newlyCollected = [{ id: '2', created_at: '2026-01-02 00:00:00', text: 'new' }];
        const merged = mergeTweets(newlyCollected, parsed.tweets);
        ...
        const meta = buildConsolidatedMeta({
            ...
            previousCollectedCount: parsed.tweets.length,
            previousMeta: parsed.meta,
            reportedCountCurrent: 987,
            collectionMethod: 'scroll-interception-resumed',
            scrollResponsesCapturedCurrent: 5,
            mergeInfo: merged.mergeInfo,
        });

        expect(meta.reported_count).toBe(987);
        expect(meta.collected_count).toBe(2);
    });
});
