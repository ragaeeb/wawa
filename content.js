...
(function () {
    ...
    async function refreshQueryIds() {
        ...
        try {
            ...
            const bundleUrls = scripts
                .map(s => s.src)
                .filter(src => src.includes('.js') && !src.includes('.json'));
            ...
            for (const url of bundleUrls.slice(0, 5)) { // Limit to 5 bundles
                try {
                    ...
                    const genericMatch = text.match(/([A-Za-z0-9_-]{20,30})[^"]*SearchTimeline|SearchTimeline[^"]*([A-Za-z0-9_-]{20,30})/);
                    ...
                    if (discoveredQueryIds.SearchTimeline) break;
                } catch (e) {
    ...
    function updateRateLimitInfo(rateLimitInfo) {
        ...
        if (rateLimitInfo.limit) rateLimitState.limit = parseInt(rateLimitInfo.limit);
        if (rateLimitInfo.remaining !== undefined) rateLimitState.remaining = parseInt(rateLimitInfo.remaining);
        if (rateLimitInfo.reset) rateLimitState.resetTime = parseInt(rateLimitInfo.reset);
        ...
        logDebug(`Rate limit update: ${rateLimitState.remaining}/${rateLimitState.limit} remaining, delay: ${Math.round(rateLimitState.dynamicDelay)}ms`);
        ...
        while (scrollCount < maxScrolls && noChangeCount < 5) {
            ...
            try {
                ...
                if (tweets.length > 50) {
                    ...
                    if (scrollCount % 5 === 0) logDebug(`Cleaning up DOM: removing ${toRemove} tweets`);
                    for (let i = 0; i < toRemove; i++) tweets[i].remove();
                }
            } catch (e) { /* ignore */ }
            ...
            const actualRetryBtn = allButtons.find(btn =>
    ...
    function extractFullTweetData(tweet, depth = 0) {
        ...
        const media = mediaSource.map(m => {
            ...
            if (m.type === "video" || m.type === "animated_gif") {
                ...
                if (variants.length > 0) item.video_url = variants[0].url;
            }
            return item;
        });
    ...
    function buildTweetRow(tweet, type) {
        ...
        if (!data) return null;
        if (type && type !== "Tweet") data.type = type;
        return data;
    }
    ...
    function getTimelineInstructions(data) {
        ...
        if (timelineV2?.instructions?.length) return timelineV2.instructions;

        const timeline = data?.data?.user?.result?.timeline?.timeline;
        if (timeline?.instructions?.length) return timeline.instructions;

        // Search Timeline path
        const searchTimeline = data?.data?.search_by_raw_query?.search_timeline?.timeline;
        if (searchTimeline?.instructions?.length) return searchTimeline.instructions;
    ...
    function extractTimeline(data, fallbackUsername) {
        ...
        for (const instruction of instructions) {
            if (instruction.type !== "TimelineAddEntries" && instruction.type !== "TimelineReplaceEntry") continue;
    ...
    function summarizeResponseShape(data) {
        ...
        if (!user) return "No user result in response.";
    ...
    async function fetchHistoricalTweetsViaSearch(csrfToken, username, oldestCollectedDate, accountCreatedDate, seenIds, updateButtonFn) {
        logInfo("Starting historical tweet search", { oldestCollected: oldestCollectedDate, accountCreated: accountCreatedDate });
        ...
        for (let i = 0; i < dateRanges.length; i++) {
            ...
            while (rangePages < 50) { // Safety limit per range
                if (abortController?.signal?.aborted) break;
                ...
                try {
                    ...
                    if (newItems.length > 0) {
                        ...
                        consecutiveEmpty = 0;
                        logDebug(`Found ${newItems.length} new tweets in range`, { total: historicalTweets.length });
                    } else {
                        consecutiveEmpty++;
    ...
    function buildGraphqlUrl(endpoint, variables, features, fieldToggles, fieldTogglesParam = "field_toggles") {
        ...
        if (fieldToggles) params.set(fieldTogglesParam, JSON.stringify(fieldToggles));
    ...
    async function fetchSelfThreads(csrfToken, collected, targetUserId, maxThreads = 50) {
        ...
        const candidates = collected.filter(t =>
            t.reply_count > 0 &&
            t.author && (t.author.id === targetUserId || t.author.id_str === targetUserId)
        );
    ...
    function getUsernameFromUrl() {
        ...
        if (match) {
            ...
            const reserved = [
                ...
                "lists", "settings", "compose", "i", "intent", "login", "logout",
    ...
    function handleLooksDone(batchCount) {
        ...
        header.style.cssText = "display: flex; align-items: center; gap: 8px; color: #4CAF50; font-weight: bold; font-size: 14px;";
    ...
    function handleRouteChange(batchCount, originalUrl) {
        ...
        // Header
        const header = document.createElement("div");
        header.style.cssText = "display: flex; align-items: center; gap: 8px; color: #FF9800; font-weight: bold; font-size: 14px;";
    ...
    function handleResumeFromFile() {
        ...
        fileInput.onchange = async (e) => {
            ...
            try {
                ...
                tweets.sort((a, b) => {
                    const dateA = parseTweetDate(a.created_at);
                    const dateB = parseTweetDate(b.created_at);
                    return dateB - dateA;
                });
    ...
    function parseTweetDate(dateStr) {
        ...
        // Try to match our format: "YYYY -MM -DD HH:MM:SS"
        const match = dateStr.match(/(\d{4})\s*-\s*(\d{2})\s*-\s*(\d{2})\s*(\d{2}):(\d{2}):(\d{2})/);
        if (match) {
    ...
    function handleRateLimitEvent(url) {
        ...
        if (document.getElementById("twexport-rl-controls")) return;
        ...
        // Header with icon
        const header = document.createElement("div");
        header.style.cssText = "display: flex; align-items: center; gap: 8px; color: #e94560; font-weight: bold; font-size: 14px;";
        ...
        const resetTime = rateLimitState.resetTime ? new Date(rateLimitState.resetTime * 1000).toLocaleTimeString() : 'unknown';
    ...
    async function runScrollExport(username, userId, csrfToken, user) {
        ...
        try {
            ...
            let collected = extractTweetsFromInterceptedResponses(userId);
    ...
    async function runExport(username) {
        ...
        try {
            const startedAt = new Date().toISOString();
            const csrfToken = getCsrfToken();
            if (!csrfToken) throw new Error("Could not find CSRF token. Are you logged in?");
            ...
            while (collected.length < maxCount) {
                if (abortController?.signal?.aborted) throw new Error("Export cancelled by user.");

                page += 1;
                updateButton(`📥 Page ${page} (${collected.length} tweets)`);
                logInfo(`Fetching page ${page}`, { collected: collected.length, cursor: cursor ? cursor.slice(0, 50) + "..." : null });
            ...
            if (shouldSearchHistorical) {
                ...
                try {
                    ...
                    logWarn("Historical search failed, continuing with timeline data", { error: err.message });
                }
