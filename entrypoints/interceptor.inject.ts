export default defineUnlistedScript(() => {
  ...
  (window as { __twexportOriginalFetch?: typeof window.fetch }).__twexportOriginalFetch =
    originalFetch;
    ...
    try {
      if (
        ...
        const rateLimitInfo = {
          ...
          remaining:
            Number.parseInt(response.headers.get("x-rate-limit-remaining") ?? "50", 10) || 50,
          reset:
  ...
  XMLHttpRequest.prototype.send = function (...sendArgs: Parameters<typeof originalSend>): void {
    this.addEventListener("load", function () {
      ...
      if (xhr.status === 401) {
        window.postMessage(
          { type: "TWEXPORT_AUTH_ERROR", payload: { url: xhr._url, status: 401 } },
          "*",
        );
        return;
      }
      ...
      try {
        ...
        if (
          ...
          const rateLimitInfo = {
            ...
            remaining:
              Number.parseInt(xhr.getResponseHeader("x-rate-limit-remaining") ?? "50", 10) || 50,
            reset:
