...
import {
  ...
  reduceExportLifecycle,
  shouldPromptLooksDone,
...
(function () {
  ...
  async function refreshQueryIds() {
    ...
    try {
      ...
      for (const script of allScripts) {
        if (
          script.src.includes("api.") ||
          script.src.includes("bundle.") ||
          script.src.includes("client.")
        ) {
      ...
      for (const url of bundleUrls.slice(0, 5)) {
        ...
        try {
          ...
          const searchMatch = text.match(
            /queryId:\s*"([^"]+)"[^}]*operationName:\s*"SearchTimeline"/,
          );
          if (searchMatch) {
  ...
  function updateRateLimitInfo(rateLimitInfo) {
    ...
    if (rateLimitInfo.remaining !== undefined)
      rateLimitState.remaining = parseInt(rateLimitInfo.remaining);
  ...
  async function scrollToLoadMore(maxScrolls = 100, delayMs = 2000) {
    ...
    logInfo(
      `Scroll loading complete: ${scrollCount} scrolls, ${interceptedResponses.length} responses captured`,
    );
  ...
  function generateDateRanges(startDate, endDate, monthsPerChunk = 6) {
    ...
    while (current > end) {
      ...
      const since =
        current < end ? end.toISOString().split("T")[0] : current.toISOString().split("T")[0];
  ...
  async function fetchSelfThreads(csrfToken, collected, targetUserId, maxThreads = 50) {
    ...
    const candidates = collected.filter(
      (t) =>
        t.reply_count > 0 &&
        t.author &&
        (t.author.id === targetUserId || t.author.id_str === targetUserId),
    );
  ...
  function showCooldownUI(duration) {
    ...
    timerDisplay.style.cssText =
      "font-size: 24px; font-weight: bold; text-align: center; margin: 4px 0;";
  ...
  function handleResumeFromFile() {
    ...
    fileInput.onchange = async (e) => {
      ...
      try {
        ...
        let username =
          data.meta?.username || data.metadata?.username || getUsernameFromUrl() || "unknown";
  ...
  async function handleScrollExport() {
    ...
    const isResumeRequest =
      params.get("twexport_resume") === "1" || Boolean(autoStartCtx?.resumeMode);
    if (isResumeRequest) {
    ...
    try {
      ...
      const consolidatedMeta = buildConsolidatedMeta({
        ...
        reportedCountCurrent:
          Number.isFinite(totalTweetsReported) && totalTweetsReported > 0
            ? totalTweetsReported
            : null,
