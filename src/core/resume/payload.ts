...
export const parseTweetDate = (dateStr: string | undefined): Date | null => {
    ...
    const customMatch = dateStr.match(/(\d{4})\s*-\s*(\d{2})\s*-\s*(\d{2})\s*(\d{2}):(\d{2}):(\d{2})/);
    if (customMatch) {
...
export const extractTweetsFromExportData = (data: unknown): TweetItem[] => {
    ...
    if (Array.isArray(payload.items)) {
        return payload.items as TweetItem[];
    }
    if (Array.isArray(payload.tweets)) {
        return payload.tweets as TweetItem[];
    }
    return [];
};
...
export const buildResumePayload = (input: {
    username: string;
    tweets: TweetItem[];
    meta: ExportMeta | null;
    savedAt?: number;
