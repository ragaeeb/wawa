...
function normalizeResumePayload(payload: unknown): ResumePayload | null {
  if (!payload || typeof payload !== "object") return null;

  const candidate = payload as Partial<ResumePayload>;
  if (!Array.isArray(candidate.tweets) || candidate.tweets.length === 0) return null;
...
function parseSerializedManifest(value: unknown): SerializedManifest | null {
...
function idbChunkKey(index: number): string {
  return `${IDB_CHUNK_PREFIX}${index}`;
}

function fallbackChunkKey(index: number): string {
  return `${FALLBACK_CHUNK_PREFIX}${index}`;
}

async function openResumeDb(indexedDbFactory: IDBFactory): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(request.error ?? new Error("Failed to open IndexedDB for resume state"));
  });
}

async function readStoreValue(db: IDBDatabase, key: string): Promise<unknown> {
  return new Promise((resolve, reject) => {
    ...
    tx.onabort = () =>
      reject(tx.error ?? new Error(`IndexedDB transaction aborted while reading key ${key}`));
  });
}

async function putResumePayload(
  indexedDbFactory: IDBFactory,
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    tx.onerror = () =>
      reject(tx.error ?? new Error("Failed to persist resume payload in IndexedDB"));
    tx.onabort = () =>
      reject(tx.error ?? new Error("IndexedDB transaction aborted while persisting payload"));
  });

  db.close();
}

async function getResumePayload(indexedDbFactory: IDBFactory): Promise<ResumePayload | null> {
  const db = await openResumeDb(indexedDbFactory);
...
async function clearResumePayload(indexedDbFactory: IDBFactory): Promise<void> {
  const db = await openResumeDb(indexedDbFactory);
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    tx.oncomplete = () => resolve();
    tx.onerror = () =>
      reject(tx.error ?? new Error("Failed to clear resume payload from IndexedDB"));
    tx.onabort = () =>
      reject(tx.error ?? new Error("IndexedDB transaction aborted while clearing payload"));
  });

  db.close();
}

export function createChromeLocalFallbackStorage(): FallbackStorage {
  return {
...
async function writeFallbackPayload(
  fallbackStorage: FallbackStorage,
  payload: ResumePayload,
): Promise<void> {
  const serialized = JSON.stringify(payload);
...
async function clearFallbackPayload(fallbackStorage: FallbackStorage): Promise<void> {
  const raw = await fallbackStorage.get<unknown>(STORAGE_KEYS.RESUME_PAYLOAD_FALLBACK);
  const manifest = parseSerializedManifest(raw);
...
export function createResumeStorage(options: ResumeStorageOptions = {}): ResumeStorage {
  const indexedDbFactory = options.indexedDbFactory;
  const fallbackStorage = options.fallbackStorage;
  const maxAgeMs = options.maxAgeMs ?? DEFAULT_MAX_AGE_MS;

  async function persist(payload: ResumePayload): Promise<boolean> {
  ...
  async function restore(targetUsername?: string): Promise<ResumePayload | null> {
    ...
    if (expected && payload.username !== expected) return null;
