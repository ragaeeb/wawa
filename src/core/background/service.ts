...
export const createChromeSettingsStore = (): SettingsStore => {
    return {
...
export const createBackgroundService = (settingsStore: SettingsStore): BackgroundService => {
    ...
    const handleMessage = async (message: RuntimeMessage): Promise<RuntimeResponseMap[keyof RuntimeResponseMap]> => {
        switch (message.type) {
            case 'log': {
